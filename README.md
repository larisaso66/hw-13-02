...
**Fail2Ban** не сработал, так как в логах отсутствуют записи о неудачных попытках аутентификации. Для срабатывания Fail2Ban нужны именно неудачные попытки входа с неверными паролями, а не просто TCP-соединения с 22 портом

В ходе выполнения задания nmap только сканировал порты, но не пытался подобрать пароли 
